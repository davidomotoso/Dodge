...
const ScrollAnimation = ({ bgChange }) => {
  ...
  return (
    <section className="xl:flex hidden gap-5 items-center justify-center mt-5 pb-4 absolute bottom-2 left-[50%]">
      ...
      ></div>
      <div
        className="bg-gray-400 h-3 w-3 rounded-full relative before:absolute before:border before:w-5 before:border-gray-400 before:top-[0.37em] before:left-3"
        ref={secondscroll}
      ></div>
      <div
        className="bg-gray-400 h-3 w-3 rounded-full 3"
        ref={thirdScroll}
      ></div>
    </section>
  );
